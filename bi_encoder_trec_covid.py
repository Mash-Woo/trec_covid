...
class ExperimentRunner:
    ...
    def __init__(self, retriever: BiEncoderRetriever):
    ...
    def run_experiment(
        ...
        for item in self.TOPIC_DATA:
            ...
            # Search each field and aggregate scores using RRF
            for field, text in fields.items():
                ...
                for rank, result in enumerate(results, start=1):
                    ...
                    doc_score[doc_id] += weight / (config.rrf_k + rank)
        ...
        # Save results
        output_file = f"{output_dir}/submission_{config.name}.csv"
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="TREC-COVID BiEncoder Retrieval System"
    )
    parser.add_argument(
        "--model",
        type=str,
    ...
    retriever = BiEncoderRetriever(model_config)
    ...
    if args.experiment:
        ...
        runner.run_experiment(config, args.output_dir)
    else:
        # Run all experiments
        runner.run_all_experiments(args.output_dir)
